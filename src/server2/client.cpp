  ...
  try {
    ...
    if (json["type"] == "client_list") {
      ...
      for (const auto& newclient : json["clients"]) {
        ...
        // client_hdl is the hdl to home server,
        // ideally it should be other server hdl
        server_list[1].clients.push_back(
            Client(clientid, clientkey, client_hdl));
      }
    }
    if (username == "admin") {
...
int main(int argc, char* argv[]) {
  ...
  ws_client::connection_ptr con =
      client_instance.get_connection("ws://localhost:9003", ec);
